...
> **macOS package (Apple Silicon)**: download the asset named `AALC_<version>_macos_arm64.zip` from the Release and unzip it to get `AALC.app`.
>
> 1. Move `AALC.app` to a **writable** location such as `/Applications` or `~/Applications`.
> 2. **Before the first launch**, run once in Terminal (with your actual path): `xattr -dr com.apple.quarantine /Applications/AALC.app`. The build is neither signed nor notarized, so Gatekeeper blocks it while the download quarantine flag is set; using right-click → Open without this step makes macOS run the app from a read-only randomized path (App Translocation), where it cannot save any data.
> 3. Open AALC.app and grant **AALC.app** Screen Recording and Accessibility permissions in System Settings → Privacy & Security (needed for foreground-mode screenshots/input and PlayCover keyboard injection).
> 4. Configuration, logs and image resources live in the data directory `~/Library/Application Support/AALC` (copied out of the app on first launch, about 90 MB), so **replacing AALC.app to upgrade keeps your settings**. When a new version is found, the app downloads the package into `update_temp` in that directory and reveals it in Finder; unzip it and replace `AALC.app` manually (no self-updater on macOS yet).
> 5. Emulator (ADB) and PlayCover modes need `adb`: `brew install android-platform-tools`; the packaged app already adds `/opt/homebrew/bin` and `/usr/local/bin` to PATH.
