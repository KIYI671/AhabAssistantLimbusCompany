...
6. zh_cn 不存在，en 存在且命中，那么当前语言是en；仅 share 存在且命中，那么当前语言不能确定
...
图片匹配并非串行"先 dark 后 default"：所有存在目标图片的路径**并发匹配**，汇总结果后比较 dark 与 default 的得分。若仅 dark 命中则主题为 dark；若 dark 存在但未命中、default 命中则主题为 default 并移除 dark 路径；若两者均命中，则比较最高匹配值，差距超过阈值（0.15）时胜出方确定为当前主题。
